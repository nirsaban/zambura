<?
	...
	if ($_GET['del'])
	{
		$updateQuery = "DELETE FROM  `gas_stations`  WHERE `index` ='".mysqli_real_escape_string($_GET['del'])."'";
	...
	if ($_POST['search'])
	{
		$query = "SELECT * FROM `gas_stations` WHERE   `title` LIKE '%".$_POST['search']."%' ORDER BY `title` ASC";
	}
	else
	{
		$query = "SELECT * FROM `gas_stations`  ORDER BY `title` ASC ";
	}
    ...
    <body id="minovate" class="appWrapper">
            ...
            <section id="content">
                    ...
                    <div class="tbox tbox-sm">
                        ...
                        <div class="tcol w-md bg-tr-white lt b-r">
                            ...
                            <div class="p-15 bg-white" style="min-height: 61px; text-align:center;">
                                ...
                                <a href="new_gas_station.php"   class="btn btn-sm btn-lightred b-0 br-2 text-strong">הוספת תחנת דלק חדש</a>
                            ...
                            <div class="p-15 collapse collapse-xs collapse-sm" id="mail-nav">
                                ...
                                <ul class="nav nav-pills nav-stacked nav-sm" id="mail-folders">
                                    <li class="active2" style="text-align:right;"><a href="gas_stations.php"> תחנות דלק <span class="pull-right badge bg-lightred"><? print $num_rows;?></span></a></li>
				...
				<?php if ($num_rows == 0) {?>
                    <div class="alert alert-warning alert-dismissable" style="direction:rtl; text-align:right;">
                            <strong>לא נמצאו תחנות דלק , <a href="new_gas_station.php" > לחצ/י כאן</a> כדי להוסיף תחנת דלק.</strong>
                    </div>								
				<?php } ?>	
                    	...
                    	<form action="" method="post">
                            <div class="input-group mb-10">
                                <span class="input-group-btn">
                                    <button class="btn btn-default" type="submit">! חפש תחנת דלק</button>
                                </span>
              ...
              <ul class="list-group no-radius no-border" id="mails-list">
				  ...
				  while ( $row = mysqli_fetch_array($result)){
					...
					$query2 = "SELECT * FROM `gas_companies` WHERE `index` = '".$row['company_id']."'   ORDER BY `title` ASC ";
                      ...
                      <div class="media">
                                            ...
                                            <div class="media-body">
                                                <div class="media-heading m-0">
                                                    <a href="edit_gas_station.php?id=<?= $row['index'];?>"  class="mr-1"><? print stripslashes($row['title']);?></a>
                                                    ...
                                                    <!--  <span class="hidden-xs"><? print date("d/m/y H:i:s", strtotime($row["date"]));?></span> -->
													 <div class="hidden-xs" style="margin-top:5px;">
                                                        ...
                                                        <a href="edit_gas_station.php?id=<?= $row['index'];?>"  ><button  class="btn btn-primary btn-ef btn-ef-7 btn-ef-7b mb-10" style="font-weight:bold; direction:rtl; background-color:#666">ערוך תחנת דלק<i class="glyphicon glyphicon-edit" style="padding-top:5px;  "></i></button>	</a>
                                                        
														<!--
														<a href="gas_stations.php?id=<?= $row['index'];?>" ><button  class="btn btn-primary btn-ef btn-ef-7 btn-ef-7b mb-10" style="font-weight:bold; direction:rtl; background-color:#28D8DA">תחנות דלק <span class="pull-right badge bg-lightred">3 </span><i class="glyphicon glyphicon-list" style="padding-top:5px;  "></i></button>	</a>
                                                        -->
														<button onClick="ConfirmDelete(<? print $row['index'];?>)" class="btn btn-primary btn-ef btn-ef-7 btn-ef-7b mb-10" style="font-weight:bold; direction:rtl">מחיקת תחנת דלק<i class="glyphicon glyphicon-trash" style="padding-top:5px; "></i></button>
													</div>
													
                                                    </span>
                                                </div>
											<div><?= stripslashes($row2['title']);?>
                                            </div>
                            ...
                            <section class="tile">
                                ...
                                <div class="tile-body">
									...
									<div class="form-group" style="direction:rtl;">
										...
										<input type="text" class="form-control" value='' name="cattitle" id="cattitle" placeholder="שם תחנת דלק" required>
									</div>    
        ...
        <script>
			...
			function ConfirmDelete(index)
			{
				...
				if (confirmbox)
				{
					window.location = "gas_stations.php?del="+index;
				}
			}	
