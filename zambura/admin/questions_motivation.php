...
if ($_POST['updatenew'])
{
    ...
    if ($_POST['catindex'] == "0")
    {
        ...
        if ($_FILES['file']['name'])
        {
            ...
            $sql= "INSERT INTO `questions_motivation` (`image`,`answer_type`)  VALUES ('".addslashes($newfile)."','".addslashes($_POST['answer_type'])."')";
    ...
    if ($_POST['catindex'] !="0")
    {
        ...
        $query5 = "SELECT * FROM `questions_motivation` WHERE `id` = '".$_POST['catindex']."'";
        ...
        if ($_FILES['file']['name']) {
            ...
            $newfile = stripslashes($row5['image']);
        }
            	$updateQuery = "UPDATE `questions_motivation`
            	SET
...
if ($_GET['del'])
{
	$sql = "DELETE FROM `questions_motivation` WHERE `id` = '".$_GET['del']."'";
	$del = mysqli_query($sql);
}


$answersArray = ["תשובה נכונה","תשובה לא נכונה"];
						...
						<div class="col-md-12" style="text-align:right; margin-top:15px; ">
                            ...
                            <section class="tile">
                                ...
                                <div class="tile-header dvd dvd-btm">
                                    ...
                                    <h1 class="custom-font"><strong>הודעות פרגון </strong></h1>
                                    <?php
                                    $rightAnswer = 0;
                                    $wrongAnswer = 0;
                                    $query20 = "SELECT * FROM `questions_motivation`   ORDER BY `id` DESC";
                                    $result20=mysqli_query($query20) or die('error connecting55');

                                    while ( $row20 = mysqli_fetch_array($result20) )
                                    {
                                        if ($row20['answer_type'] == 0)
                                            $rightAnswer++;

                                        if ($row20['answer_type'] == 1)
                                            $wrongAnswer++;
                                    }
                                    ?>
                                    <div style="direction: rtl;">
                                        <div>תשובות נכונות: <?= $rightAnswer;?></div>
                                        <div>תשובות לא נכונות: <?= $wrongAnswer;?></div>
                                    </div>
                                                            ...
                                                            <div class="p-15 bg-white" style="min-height: 61px; text-align:center;">
                                                                ...
                                                                <button class="btn btn-sm btn-default pull-right visible-sm visible-xs" data-toggle="collapse" data-target="#mail-nav" aria-expanded="false" aria-controls="collapseExample"><i class="fa fa-bars"></i></button>
                                                                <a href="#"  onClick="addCatagory(0,'','הוספת הודעת פרגון')" data-toggle="modal" data-target="#tipmodal" data-options="splash-2 splash-ef-9" class="btn btn-sm btn-lightred b-0 br-2 text-strong">הוספת הודעת פרגון</a>
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table hover" id="basic-usage2" style="width:100%" align="center">
                                            ...
                                            <tr>
                                                ...
                                                <th  style="text-align:center; width:30%">סוג</th>
										...
										<?
											$i = 0;
											$query = "SELECT * FROM `questions_motivation`   ORDER BY `id` DESC";
                                        ...
                                        <tr>
                                            ...
                                            <td class="actions">
													...
													<button  onClick="editCatagory('<?= $row['id'];?>','עריכת הודעת פרגון')" data-toggle="modal" data-target="#tipmodal" data-options="splash-2 splash-ef-9"  class="btn btn-info btn-rounded btn-icon-only btn-ef btn-ef-7 btn-ef-7g mb-10"><i class="fa fa-pencil"></i> Add <i class="after fa fa-plus"></i></button>
											</td>


                                            <td align="center" ><?= $answersArray[$row['answer_type']];?></td>
                                            ...
                                            <td align="center" ><?= $row['id'];?></td>
                                                ...
                                                <input type="hidden" id="dbanswer_type_<?= $row['id'];?>" name="dbanswer_type_<?= $row['id'];?>" value='<?= stripslashes($row['answer_type']);?>'>
										<?
										}
										?>										
	...
	?>
                    ...
                    <!-- tile -->
                            <section class="tile">
                                ...
                                <div class="tile-body">
                                    ...
                                    <div class="form-group" style="direction:rtl;">
                                        <label for="exampleInputEmail1">סוג תשובה:</label>
                                        <select name="answer_type" id="answer_type" class="form-control" required>
                                            <option value="0">נכון</option>
                                            <option value="1">לא נכון</option>
                                        </select>
                                    </div>
        ...
        ================================================ -->
         <script>
			...
			{
				var answer_type = $('#dbanswer_type_'+index).val();
			...
			function ConfirmDelete(index)
			{
				...
				if (confirmbox)
				{
					window.location = "questions_motivation.php?del="+index;
				}
			}
