<?
	...
	if ($_POST['updatenew'])
	{
		if ($_POST['catindex'] =="0")
		{
			...
			$sql= "INSERT INTO `gas_companies` (`title`,`image`)  VALUES ('".addslashes($_POST['cattitle'])."','".addslashes($newfile)."')";
			$InsertSql = mysqli_query($sql);				
				
			}
			else 
			{
			$sql= "INSERT INTO `gas_companies` (`title`)  VALUES ('".addslashes($_POST['cattitle'])."')";
			$InsertSql = mysqli_query($sql);				
			}
		}
		...
		if ($_POST['catindex'] !="0")
		{
			...
			if ($_FILES['file']['name'])
			{
				...
				$updateQuery = "UPDATE `gas_companies`
				SET  
	...
	if ($_GET['del'])
	{
		$updateQuery = "DELETE FROM  `gas_companies`  WHERE `index` ='".mysqli_real_escape_string($_GET['del'])."'";
		$update =  mysqli_query($updateQuery) or die(mysqli_error());  
	}
	...
	if ($_POST['search'])
	{
		$query = "SELECT * FROM `gas_companies` WHERE   `title` LIKE '%".$_POST['search']."%' ORDER BY `title` ASC";
	}
	else
	{
    ...
    <body id="minovate" class="appWrapper">
            ...
            <section id="content">
                    ...
                    <div class="tbox tbox-sm">
                        ...
                        <div class="tcol w-md bg-tr-white lt b-r">
                            ...
                            <div class="p-15 bg-white" style="min-height: 61px; text-align:center;">
                                ...
                                <a href="#" onClick="addCatagory(0,'','הוספת חברת דלק חדשה')" data-toggle="modal" data-target="#splash" data-options="splash-2 splash-ef-9" class="btn btn-sm btn-lightred b-0 br-2 text-strong">הוספת חברת דלק חדשה</a>
                            ...
                            <div class="p-15 collapse collapse-xs collapse-sm" id="mail-nav">
                                ...
                                <ul class="nav nav-pills nav-stacked nav-sm" id="mail-folders">
                                    <li class="active" style="text-align:right;"><a href="gas_station_catagories.php"> מאפיינים תחנות דלק </a></li>
                                ...
                                <ul class="nav nav-pills nav-stacked nav-sm" id="mail-folders">
                                    <li class="active2" style="text-align:right;"><a href="gas_companies.php"> חברות דלק <span class="pull-right badge bg-lightred"><? print $num_rows;?></span> </a></li>
				...
				<?php if ($num_rows == 0) {?>
                    <div class="alert alert-warning alert-dismissable" style="direction:rtl; text-align:right;">
                            <strong>לא נמצאו חברות דלק , <a href="#" onClick="addCatagory(0,'','הוספת חברת דלק חדשה')" data-toggle="modal" data-target="#splash" data-options="splash-2 splash-ef-9"> לחצ/י כאן</a> כדי להוסיף חברת דלק.</strong>
                    </div>								
				<?php } ?>	
                    	...
                    	<form action="" method="post">
                            <div class="input-group mb-10">
                                <span class="input-group-btn">
                                    <button class="btn btn-default" type="submit">! חפש חברת דלק</button>
                                </span>
                  ...
                  <li class="list-group-item b-primary" style="direction:rtl; text-align:right;">
                      ...
                      <div class="media">
                                            <div class="pull-right">
												...
												<div class="thumb thumb-sm" style="width:65px;" onClick='editCatagory("<? print $row['index'];?>","עריכת חברת דלק","<? print stripslashes($row['employee']);?>","<? print stripslashes($row['Derug']);?>");'  data-toggle="modal" data-target="#splash" data-options="splash-2 splash-ef-9">
                                            ...
                                            <div class="media-body">
                                                <div class="media-heading m-0">
                                                    <a href="#" onClick='editCatagory("<? print $row['index'];?>","עריכת חברת דלק","<? print stripslashes($row['employee']);?>","<? print stripslashes($row['Derug']);?>");'   data-toggle="modal" data-target="#splash" data-options="splash-2 splash-ef-9" class="mr-1"><? print stripslashes($row['title']);?></a>
													<!--
                                                    ...
                                                    <!--  <span class="hidden-xs"><? print date("d/m/y H:i:s", strtotime($row["date"]));?></span> -->
													 <div class="hidden-xs" style="margin-top:5px;">
                                                        ...
                                                        <a href="#" onClick='editCatagory("<? print $row['index'];?>","עריכת חברת דלק","<? print stripslashes($row['employee']);?>","<? print stripslashes($row['Derug']);?>");'  data-toggle="modal" data-target="#splash" data-options="splash-2 splash-ef-9"><button  class="btn btn-primary btn-ef btn-ef-7 btn-ef-7b mb-10" style="font-weight:bold; direction:rtl; background-color:#666">ערוך חברת דלק<i class="glyphicon glyphicon-edit" style="padding-top:5px;  "></i></button>	</a>
                                                        <button onClick="ConfirmDelete(<? print $row['index'];?>)" class="btn btn-primary btn-ef btn-ef-7 btn-ef-7b mb-10" style="font-weight:bold; direction:rtl">מחיקת חברת דלק<i class="glyphicon glyphicon-trash" style="padding-top:5px; "></i></button>
													</div>
                            ...
                            <section class="tile">
                                ...
                                <div class="tile-body">
									...
									<div class="form-group" style="direction:rtl;">
										<label for="exampleInputEmail1">שם חברת הדלק:</label>
										<input type="text" class="form-control" value='' name="cattitle" id="cattitle" placeholder="שם חברת דלק" required>
									</div>    
        ...
        <script>
			...
			function ConfirmDelete(index)
			{
				...
				if (confirmbox)
				{
					window.location = "gas_companies.php?del="+index;
				}
			}	
