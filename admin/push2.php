...
if ($_GET['type'] == "1")
{
	$query = "SELECT * FROM `users`  WHERE `gender` = '0' AND `soldier` = '1' AND  `push_id` != '' ORDER BY `index` DESC";
}
if ($_GET['type'] == "2")
{
	$query = "SELECT * FROM `users`  WHERE `gender` = '1' AND `soldier` = '1' AND  `push_id` != '' ORDER BY `index` DESC";
}
if ($_GET['type'] == "3")
{
	$query = "SELECT * FROM `users`  WHERE   `soldier` = '1' AND  `push_id` != '' ORDER BY `index` DESC";
}
if ($_GET['type'] == "4")
{
	$query = "SELECT * FROM `users`  WHERE `gender` = '0' AND `soldier` = '0' AND   `push_id` != '' ORDER BY `index` DESC";
}
if ($_GET['type'] == "5")
{
	$query = "SELECT * FROM `users`  WHERE `gender` = '1' AND `soldier` = '0' AND   `push_id` != '' ORDER BY `index` DESC";
}
if ($_GET['type'] == "6")
{
	$query = "SELECT * FROM `users`  WHERE `gender` = '1' AND `soldier` = '0' AND   `push_id` != '' ORDER BY `index` DESC";
...
if ($_GET['type'] == "7")
{
	$query = "SELECT * FROM `users`  WHERE     `push_id` != '' ORDER BY `index` DESC";
}
$result=mysqli_query($query) or die('error connecting22'); 
    ...
    <body id="minovate" class="appWrapper">
                    ...
                    <div class="tbox tbox-sm">
                        ...
                        <div class="tcol w-md bg-tr-white lt b-r">
                            ...
                            <div class="p-15 collapse collapse-xs collapse-sm" id="mail-nav">
                                ...
                                <ul class="nav nav-pills nav-stacked nav-sm" id="mail-folders" >
                                    <li class="active" ><a href="push.php?type=<?= $_GET['type'];?>" class="pull-right" style="width:100%; text-align:right">שליחת הודעות <span class="pull-left badge bg-lightred"><? print $num_rows1;?></span></a></li>
									
                                </ul>

                                <h5 class="b-b p-10 text-strong" style="text-align:right;">מיון</h5>
                                <ul class="nav nav-pills nav-stacked nav-sm" id="mail-labels" >
								<?php
								foreach ($TypeArray as $key=>$values)
								{
									if ($_GET['type'] == $key)
										print '<li><a href="push.php?type='.$key.'" style="text-align:right"><i class="fa fa-fw fa-circle text-red pull-right" style="padding-top:3px;"></i>'.$values.'</a></li>';
									else
										print '<li><a href="push.php?type='.$key.'" style="text-align:right"><i class="fa fa-fw fa-circle text-primary pull-right" style="padding-top:3px;"></i>'.$values.'</a></li>';
                        ...
                        <div class="tcol">
							...
							<div class="alert alert-warning alert-dismissable" style="direction:rtl; text-align:right;">
									<strong>לא נמצאו <?= $tableName;?></strong>
							</div>								
							<?php
							}
							?>	
						...
						<div class="alert alert-success" style="direction:rtl; text-align:right;">
							<strong>הודעה נשלחה בהצלחה ל <?= $g;?> <?= $tableName;?>.</strong>
						</div>
						<?
						}
						?>
        ...
        <script>
			...
			function ConfirmDelete(index)
			{
				...
				if (confirmbox)
				{
					window.location = "push.php?del="+index;
				}
			}			
        </script>
        <!--/ Page Specific Scripts -->
