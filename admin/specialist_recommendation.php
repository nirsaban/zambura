...
if ($_POST['updatenew'])
{
    ...
    if ($_POST['catindex'] == "0")
    {
        ...
        $sql= "INSERT INTO `specialist_recommendation` (`title`,`min_points`,`max_points`)  VALUES (
                '".addslashes($_POST['cattitle'])."',
                '".addslashes($_POST['min_points'])."',
    ...
    if ($_POST['catindex'] !="0")
    {
        $updateQuery = "UPDATE `specialist_recommendation` 
		SET 
		`title` ='".addslashes($_POST['cattitle'])."',
		`min_points` ='".addslashes($_POST['min_points'])."',
...
if ($_GET['del'])
{
	$sql = "DELETE FROM `specialist_recommendation` WHERE `id` = '".$_GET['del']."'";
    ...
    <body id="minovate" class="appWrapper">
        ...
        <div id="wrap" class="animsition">
            ...
            <div id="controls">
				...
				<?
                $submenu = 'general';
                $menu = 'specialist_recommendation';
						...
						<div class="col-md-12" style="text-align:right; margin-top:15px; ">
                            ...
                            <section class="tile">
                                ...
                                <div class="tile-header dvd dvd-btm">
                                    ...
                                    <h1 class="custom-font"><strong>המלצת המומחה  </strong></h1>
                                                            ...
                                                            <div class="p-15 bg-white" style="min-height: 61px; text-align:center;">
                                                                ...
                                                                <button class="btn btn-sm btn-default pull-right visible-sm visible-xs" data-toggle="collapse" data-target="#mail-nav" aria-expanded="false" aria-controls="collapseExample"><i class="fa fa-bars"></i></button>
                                                                <a href="#"  onClick="addCatagory(0,'','הוספת המלצת המומחה')" data-toggle="modal" data-target="#tipmodal" data-options="splash-2 splash-ef-9" class="btn btn-sm btn-lightred b-0 br-2 text-strong">הוספת המלצת המומחה</a>
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table hover" id="basic-usage2" style="width:100%" align="center">
                                            ...
                                            <tr>
                                                ...
                                                <th  style="text-align:center; width:25%">מקסימום נקודות</th>
                                                <th  style="text-align:center; width:25%">מינימום נקודות</th>
                                                <th  style="text-align:center; width:35%">כותרת</th>
										...
										<?
											$i = 0;
											$query = "SELECT * FROM `specialist_recommendation`  ORDER BY `title` ASC";
                                        ...
                                        <tr>
                                            ...
                                            <td class="actions">
													...
													<button  onClick="editCatagory('<?= $row['id'];?>','עריכת המלצה')" data-toggle="modal" data-target="#tipmodal" data-options="splash-2 splash-ef-9"  class="btn btn-info btn-rounded btn-icon-only btn-ef btn-ef-7 btn-ef-7g mb-10"><i class="fa fa-pencil"></i> Add <i class="after fa fa-plus"></i></button>
											</td>

                                            <td align="center" ><?= stripslashes($row['max_points']);?></td>
                                            <td align="center" ><?= stripslashes($row['min_points']);?></td>
                                            ...
                                            <td align="center" ><?= $row['id'];?></td>
                                                ...
                                                <input type="hidden" id="dbmin_points_<?= $row['id'];?>" name="dbmin_points_<?= $row['id'];?>" value='<?= stripslashes($row['min_points']);?>'>
                                                <input type="hidden" id="dbmax_points_<?= $row['id'];?>" name="dbmax_points_<?= $row['id'];?>" value='<?= stripslashes($row['max_points']);?>'>
										<?
										}
										?>										
                            ...
                            <section class="tile">
                                ...
                                <div class="tile-body">
                                    ...
                                    <div class="form-group" style="direction:rtl;">
                                        <label for="exampleInputEmail1">המלצת המומחה:</label>
                                        <input type="text" class="form-control" value='' name="cattitle" id="cattitle" placeholder="המלצת המומחה" required>
                                    </div>

                                    <div class="form-group" style="direction:rtl;">
                                        <label for="exampleInputEmail1">מינימום נקודות:</label>
                                        <input type="number" class="form-control" value='' name="min_points" id="min_points" placeholder="מינימום נקודות" required>
                                    </div>

                                    <div class="form-group" style="direction:rtl;">
                                        <label for="exampleInputEmail1">מקסימום נקודות:</label>
                                        <input type="number" class="form-control" value='' name="max_points" id="max_points" placeholder="מקסימום נקודות" required>
                                    </div>
        ...
        ================================================ -->
         <script>
			...
			function editCatagory(index,title)
			{
                ...
                var minpoints = $('#dbmin_points_'+index).val();
                var maxpoints = $('#dbmax_points_'+index).val();
                ...
                $("#min_points").val(minpoints);
                $("#max_points").val(maxpoints);
			...
			function ConfirmDelete(index)
			{
				...
				if (confirmbox)
				{
					window.location = "specialist_recommendation.php?del="+index;
				}
			}
